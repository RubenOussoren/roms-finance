...
class Family::AutoMerchantDetectorTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @llm_provider = mock
    concept = mock
    concept.stubs(:providers).returns([ @llm_provider ])
    Provider::Registry.stubs(:for_concept).with(:llm).returns(concept)
  end
  ...
  test "auto detects transaction merchants" do
    ...
    assert_equal "https://logo.clearbit.com/mcdonalds.com", txn1.reload.merchant.logo_url
    assert_equal "https://logo.clearbit.com/chipotle.com", txn2.reload.merchant.logo_url
