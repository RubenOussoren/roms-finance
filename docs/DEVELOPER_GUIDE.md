...
├── design/
│   └── claude-design-workflow.md # Using Claude Design (claude.ai/design) with this app
├── hosting/
│   └── docker.md          # Self-hosting with Docker Compose
└── reviews/               # Post-phase code review reports (generated by /phase-review skill)
```
...
| Design system workflows (Claude Design)    | `docs/design/`            | Maintainer (update when DS tokens change)      |
...
| Post-phase review reports                  | `docs/reviews/`           | Reviewer (generated by `/phase-review` skill)  |
...
Some upgrades ship data-model changes that need a one-time rake task beyond `bin/rails db:migrate`.

### `data_migration:backfill_equity_sales`

```bash
bin/rails data_migration:backfill_equity_sales
```

Required after the per-grant sold-unit tracking change. Migrates historical GSU/RSU sales — recorded via the old `create_transfer_from` rule or as manual positive-amount Transaction entries — into the new `EquityGrantSale` table. FIFO-allocates units to grants by `grant_date` using historical security prices. Idempotent.

Without this step, Equity Compensation accounts with legacy sales will overestimate balance because vesting valuations don't know which units were sold.
