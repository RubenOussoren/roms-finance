...
```bash
# Download the sample compose.yml file from the ROMS Finance Github repository
...
If everything is working, you will see the ROMS Finance login screen.
...
Most self-hosting users will want the ROMS Finance app to run in the background on their computer so they can access it at all times. To do this, hit `Ctrl+C` to stop the running process, and then run the following command:
...
If you find bugs or have a feature request, please [open an issue](https://github.com/RubenOussoren/roms-finance/issues).

## How to update your app

The mechanism that updates your self-hosted ROMS Finance app is the GHCR (Github Container Registry) Docker image that you see in the `compose.yml` file:

```yml
image: ghcr.io/rubenoussoren/roms-finance:latest
```

We recommend using one of the following images, but you can pin your app to whatever version you'd like (see [packages](https://github.com/RubenOussoren/roms-finance/pkgs/container/roms-finance)):

- `ghcr.io/rubenoussoren/roms-finance:latest` (latest commit)
- `ghcr.io/rubenoussoren/roms-finance:stable` (latest release)
...
```bash
cd ~/docker-apps/roms-finance # Navigate to whatever directory you configured the app in
...
```yml
image: ghcr.io/rubenoussoren/roms-finance:stable
```
...
If you are trying to get ROMS Finance started for the **first time** and run into database connection issues, it is likely because Docker has already initialized the Postgres database with a _different_ default role (usually from a previous attempt to start the app).

If you run into this issue, you can optionally **reset the database**.

**PLEASE NOTE: this will delete any existing data that you have in your ROMS Finance database, so proceed with caution.**  For first-time users of the app just trying to get started, you're generally safe to run the commands below.

By running the commands below, you will delete your existing ROMS Finance database and "reset" it.

```
docker compose down
docker volume rm roms-finance_postgres-data # this is the name of the volume the DB is mounted to
docker compose up
docker exec -it roms-finance-db-1 psql -U roms_user -d roms_production -c "SELECT 1;" # This will verify that the issue is fixed
```
...
```bash
docker exec roms-finance-db-1 pg_dump -U ${POSTGRES_USER:-roms_user} ${POSTGRES_DB:-roms_production} | gzip > roms-finance_backup_$(date +%Y%m%d).sql.gz
```
